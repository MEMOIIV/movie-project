...
async function getTrendingTV() {
    ...
    for (let page = 1; page < numberOfPagesToFetch; page++) {
        request.push(axios.get(`https://api.themoviedb.org/3/discover/tv?api_key=ab6f02890a894dfd18b04c025b5de2eb&page=${page}`))
    }
    const responses = await Promise.all(request)
    const allDataTv =responses.flatMap((response) => {return response.data.results})
    const uniqueTVShows = Array.from(new Set(allDataTv.map(tvShow => tvShow.id)))
    .map(id => {
        return allDataTv.find(tvShow => tvShow.id === id);
    });
    setTimeout(() => {
    setAllTV(uniqueTVShows );
    }, 200);
}
