...
export default function AllMovies() {
    ...
    const numberOfPagesToFetch = 10 ;

    async function getAllMovies(){
        const request =[]
        for (let page = 1; page < numberOfPagesToFetch; page++) {
            request.push(axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=ab6f02890a894dfd18b04c025b5de2eb&page=${page}`))
        }
        const responses = await Promise.all(request)
        const allDataMovies =responses.flatMap((response) => {return response.data.results})
        const uniquemovie = Array.from(new Set(allDataMovies.map(tvShow => tvShow.id)))
    .map(id => {
        return allDataMovies.find(tvShow => tvShow.id === id);
    });
        setTimeout(()=>{
            setAllMovies(uniquemovie)
        }, 200)
    }
